...
// This component displays the user's profile bar with options to edit profile and logout
  ...
  return (
    <div className="w-full px-2 mb-3 relative">
      <div className="flex items-center gap-2 bg-[#23272a] rounded-lg p-2 cursor-pointer w-full" onClick={() => setShowProfileMenu(!showProfileMenu)}>
        <div className="bg-[#5865f2] rounded-full w-9 h-9 flex items-center justify-center overflow-hidden">
          {user.profilePicture ? (
            <img
      ...
      {showProfileMenu && (
        <div className="absolute left-0 bottom-full mb-2 z-50 bg-[#202225] border border-[#5865f2] rounded-lg shadow-lg w-40 p-2 flex flex-col gap-2 animate-fade-in">
          <Button variant="ghost" className="justify-start" onClick={() => { setShowProfileMenu(false); navigate('/edit-profile'); }}>Edit profile</Button>
